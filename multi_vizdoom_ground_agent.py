...
class VizDoomSingleGroundAgent:
    def __init__(self, trail, wad, seed=0, game_args=[]):
        self.trail = trail
    ...
    def trail_following(self, future_state, future_position):
        cv2.imshow('image', future_state)
        cv2.waitKey(10)

        repeat = 1

        current_angle = self.current_pose[3]
        if (current_angle <= -180):
            current_angle += 360;
        if (current_angle > 180):
            current_angle -= 360;

        distance = math.sqrt((future_position[0] - self.current_pose[0]) ** 2 +
                             (future_position[1] - self.current_pose[1]) ** 2 + 
                             (future_position[2] - self.current_pose[2]) ** 2)
        if (distance < 20):
            action = constants.ACTION_MOVE_FORWARD
            return action, repeat

        rad = math.atan2(future_position[1] - self.current_pose[1], future_position[0] - self.current_pose[0])
        deg = math.degrees(rad)

        direction = current_angle - deg
        if (direction > 6.):
    ...
    def search(self):
        ...
        if (len(self.random_ongoing_actions) > 0):
            ...
            future_state, future_position, score, self.last_matched = self.trail.find_closest_ground_waypoint(self.current_pose, backward=False)
...
#                action, repeat = self.local_random_search()
                ...
                action, repeat = self.trail_following(future_state, future_position)
...
class MultiVizDoomGroundAgent(MultiAgent):
    def __init__(self, wad, lmp):
        super(MultiVizDoomGroundAgent, self).__init__(None, None)
        self.wad = wad
        self.lmp = lmp
        ...
        for i in range(constants.MULTI_NUM_AGENTS):
            self.agents.append(VizDoomSingleGroundAgent(self.trail, self.wad, self.seed, game_args=[]))
